...
# 2. VERİLERİ ÇEK VE HAZIRLA
SEMBOL_LISTESI = ['AAPL', 'MSFT', 'AMZN', 'NVDA', 'GOOGL', 'META', 'TSLA', 'BRK-B', 'JPM', 'JNJ', 'V', 'PG', 'UNH', 'HD', 'MA', 'CVX', 'ABBV', 'MRK', 'PEP', 'KO', 'LLY', 'AVGO', 'COST', 'TMO', 'MCD', 'DIS', 'CSCO', 'ACN', 'INTC', 'XOM']
dfs = {}
print("\n[VERI] Yahoo Finance üzerinden 10 yıllık tarihsel veriler çekiliyor (1D Mumlar)...")
for sembol in SEMBOL_LISTESI:
    try:
        df = veriyi_oku_ve_ozellikleri_hesapla(sembol, backtest_10y=True)
    ...
    for gun in all_dates:
        ...
        for pos in aktif_pozisyonlar:
            ...
            if pos.get('yon', 'LONG') == 'LONG':
                ...
                hedef_fiyati = pos['entry_price'] + (pos['entry_atr'] * 4.0)
        ...
        for sembol in predict_data:
            ...
            # Master AI Karar Motoru
            hisse_sektoru = SEKTOR_HARITASI.get(sembol, "TEKNOLOJI")
            hacim_anomalisi_var_mi = float(row.get('Hacim_Anomalisi', 0.0)) > 0
            ai_karari, sinyal_sebebi = master_ai_karar_motoru(
                ai_prob_long, ai_prob_short, spot_fiyat, ema200, 
                float(row.get('ADX', 25.0)), hisse_sektoru, "BOGA", al_esigi, hacim_anomalisi_var_mi
            )
        ...
        for aday in adaylar:
            ...
            # Stop seviyesi (AFML: 1.5 ATR)
            if yon == 'LONG':
                stop_loss = close_price - (atr * 1.5)
            else:
                stop_loss = close_price + (atr * 1.5)
            
            ai_basari_ihtimali = aday['ai_prob'] / 100.0
            if ai_basari_ihtimali < 0.51:
                continue
                
            # Kısmi Kelly + Exposure Cap
            dinamik_risk_yuzdesi = max(0.01, min(0.04, (ai_basari_ihtimali - 0.50) * 0.50))
            riske_edilen_dolar = bakiye * dinamik_risk_yuzdesi
            
            risk_basina = abs(close_price - stop_loss)
            if risk_basina > 0:
                adet = int(riske_edilen_dolar / risk_basina)
                maksimum_sermaye_limiti = bakiye * 0.20
                if (adet * close_price) > maksimum_sermaye_limiti:
                    adet = int(maksimum_sermaye_limiti / close_price)
            else:
                adet = 0
